...
const Home: NextPage = () => {
  ...
  // Execute code
  function runCode() {
    // TODO: Why does referencing ref directly in runPython return "code not defined?"
    let input = code.current
    output.current = pyodide.current.runPython(input)
        ...
        <div className="grid flex-1 w-screen grid-cols-2 gap-6 px-4 py-4 h-content">
          ...
          <div className="flex flex-col gap-6">
            <CodeMirror
              ...
              height="100%"
              className="relative h-full overflow-hidden rounded h-2/3"
              onChange={(value) => (code.current = value)}
            />

            <div className="px-4 py-4 rounded bg-slate-800 text-slate-100 h-1/3">
              {output.current}
            </div>
          </div>
        </div>
